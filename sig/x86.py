...
class X86(Sig):
    ...
    def valid_loc(self, offset: int):
        ...
        return offset > 0x1000 and (((others:=self.bv.get_functions_containing(offset)) and all(o.start != self.func.start and (inst:=o.get_instruction_containing_address(offset)) and inst == offset for o in others)) or (self.bv.get_data_var_at(offset) is not None and next(self.bv.get_code_refs(offset), None) is not None))
        #DataVariable is falsey, but List and Optional works


    def calcrel(self, d: CsInsn):
        mask = bytes(d.size)
        ...
        if d.disp_offset: #consider references - any fs address, any relative memory accesses that's valid in program scope (see valid_loc def)
            m = b'\xFF' if any(op.type == X86_OP_MEM and (op.reg in [X86_REG_FS, X86_REG_GS] or (op.value.mem.base == X86_REG_RIP and self.valid_loc(op.value.mem.disp + d.address + d.size))) for op in d.operands) else b'\0'
        ...
        if d.imm_offset: #references in imm just points directly to addresses
            m = b'\xFF' if any(op.type == X86_OP_IMM and self.valid_loc(op.imm) for op in d.operands) else b'\0'
    ...
    def calc_func_metadata(self) -> tuple[str, bytes, bytes]:
        ranges = self.func.address_ranges
